@php
    ...
    $cardClass = $plan->is_featured
        ? 'rounded-[20px] border-2 border-brand-primary bg-white/15 p-8 shadow-[0_10px_40px_rgba(27,94,32,0.14)] backdrop-blur-sm transition-all duration-300 hover:-translate-y-2.5 hover:shadow-2xl flex h-full min-h-0 w-full flex-1 flex-col gap-6'
        : 'rounded-[20px] border border-white/35 bg-white/15 p-8 backdrop-blur-sm transition-all duration-300 hover:-translate-y-2.5 hover:shadow-2xl flex h-full min-h-0 w-full flex-1 flex-col gap-6';
@endphp
<div class="{{ $cardClass }}">
    ...
    <ul class="list-none min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain text-brand-dark [scrollbar-width:thin]" role="list">
